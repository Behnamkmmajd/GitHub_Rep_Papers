...
\begin{document}
	...
	\end{equation}
	where $\tau$, $\mathbf{U}$, $P$, $\kappa$, and $\hat{j}$ are the non-dimensional time, velocity vector, pressure, interface curvature, and unit vector along the Y-axis in the 3D \textit{Cartesian} and Z-axis in the \textit{axisymmetric} coordinate systems, respectively. Whereas, R is the regularization term. Utilizing the bubble diameter as the length scale $D$, the gravitation acceleration $g$, velocity scale $u_c=\sqrt{gD}$, and time scale $t_c=D/u_c$, various non-dimensional variables are given as
	\begin{equation*}
		\mathbf{X}=\frac{\mathbf{x}}{D}; \mathbf{U}=\frac{\mathbf{u}}{u_c}; \tau=\frac{t}{t_c}; P=\frac{p}{\rho_1u_c^2}
	\end{equation*}
	and the corresponding non-dimensional governing parameters are (replace the Fr and We number with Mo number)
	\begin{equation*}
	...
	\begin{figure}[!ht]
		\centering
		\begin{overpic}[scale=0.5,trim={0.1cm 0.5cm 18cm 0cm},clip]{Figure1.pdf}
			\put(5,92){\textbf{(a)}}
			\put(35,92){\textbf{(b)}}
		\end{overpic}
	...
	\begin{table}[!htbp]
		\begin{center}
			...
			\begin{tabular}{|>{\centering\arraybackslash}m{1.2cm}|c|c|c|m{2.4cm}|c|}
				...
				\hline
				C$_1$ & 20 & 20 & $ 0.30$ & $\includegraphics[scale=0.13,trim={10.5cm 23cm 25cm 5cm},clip]{C1.eps}$& 400
				\\
				\hline
				C$_2$ & 40 & 40 & $ 0.40$ & $\includegraphics[scale=0.1,trim={8cm 23cm 25cm 5cm},clip]{C2.eps}$& 400

				\\
				\hline
				C$_3$ & 100 & 100 & $ 0.63$ & $\includegraphics[scale=0.1,trim={8cm 23cm 25cm 5cm},clip]{C3.eps}$& 400
				\\

				\hline
				C$_4$ & 168.5 & 2.2 & $ 0.027$& $\includegraphics[scale=0.05,trim={10cm 10cm 8cm 8cm},clip]{C4.eps}$& 490
				\\
				\hline
				C$_5$ & 168.5 & 12 & $ 0.027$& $\includegraphics[scale=0.05,trim={10cm 10cm 8cm 10cm},clip]{C5.eps}$& 600
				\\
				\hline
				\hline
	...
	\subsection{Spectral compressibility of observation values}
	In this section, the gas volume fraction field, velocity field, and the Lagrangian position matrix are analysed in Eulerian, Lagrangian, and \hl{moving-Eulerian} frameworks by means of spectral compressibility metrics. The effectiveness of each framework is evaluated by how rapidly the snapshot energy is concentrated into a small number of modes, and how ``spread'' the spectrum remains across the retained modes.
	
	Using the singular values $\mathcal{S}_j$ obtained from the SVD of the data matrix $\mathcal{D}$ (sorted in descending order), the cumulative energy content of the first $r$ modes is defined as:
	%
	\begin{subequations}\label{eq 12}
		\begin{align}
			E_{\mathrm{cum}}(r) = \frac{\sum\limits_{j=1}^{r} \mathcal{S}_j^2}{\sum\limits_{j=1}^{m}\mathcal{S}_j^2}, \label{eq 12a} \\
			H_{\mathrm{norm}} = -\frac{1}{\ln(m)}\sum\limits_{j=1}^{m} p_j \ln(p_j), 
			\qquad p_j = \frac{\mathcal{S}_j^2}{\sum\limits_{k=1}^{m}\mathcal{S}_k^2}, \label{eq 12b}
		\end{align}
	\end{subequations}
	%
	where $E_{\mathrm{cum}}(r)\in[0,1]$ measures the accumulated energy up to rank $r$, and $H_{\mathrm{norm}}\in[0,1]$ is the normalised spectral entropy (smaller values indicate stronger energy concentration into fewer modes). In addition, $r_{99}$ is reported as the minimum ranks required to satisfy $E_{\mathrm{cum}}(r)\geq 0.99$. These metrics provide a compact and method-independent assessment of how suitable the data are for low-rank representation in each reference frame.
	
	\begin{figure}[!tbp]
		\centering
		\begin{subfigure}[t]{1.12\textwidth}
			\centering
			\includegraphics[width=\linewidth,
			trim={4.7cm 0cm 0cm 0cm},clip]{Figure3.eps}
		\end{subfigure}
		\caption{Spectral compressibility metrics for the gas volume fraction, velocity, and Lagrangian position observation values in Eulerian, Lagrangian, and \hl{moving-Eulerian} frameworks for the rising bubble a) case C$_1$ and b) case C$_3$. The results are summarised using the cumulative energy content $E_{\mathrm{cum}}(r)$, the ranks $r_{95}$ and $r_{99}$, and the normalised spectral entropy $H_{\mathrm{norm}}$. Red, blue, and green colors correspond to the Eulerian, Lagrangian, and \hl{moving-Eulerian} frameworks, respectively.}
		\label{Fig3}
	\end{figure}
	
	The spectral compressibility of the observation values is assessed in the Eulerian, Lagrangian, and \hl{moving-Eulerian} frameworks as shown in figure~\ref{Fig3}, where the left and right panels correspond to the cases C$_1$ (Re=20, Bo=20) and C$_3$ (Re=100, Bo=100), respectively. Figure~\ref{Fig3} presents the cumulative energy behaviour together with the summary metrics $r_{95}$, $r_{99}$, and $H_{\mathrm{norm}}$ for the gas volume fraction (EG, LG, MEG), velocity (EV, LV, MEV), and the Lagrangian position variable (LP).
	
	Considering the results of the bubble C$_1$ with small deformation, the gas volume fraction in the Eulerian framework is significantly less compressible than in the moving frames. In particular, EG requires $r_{99}=49$ modes with $H_{\mathrm{norm}}=0.4321$, while LG reduces this to $r_{99}=9$ and $H_{\mathrm{norm}}=0.2283$, and MEG further reduces it to $r_{99}=5$ with $H_{\mathrm{norm}}=0.1162$. This highlights that transforming the data to a moving frame strongly concentrates the energy of the gas volume fraction snapshots into fewer dominant modes.
	
	The analysis of the gas volume fraction for the bubble C$_3$ shown in figure~\ref{Fig3} supports the established patterns for C$_1$, with an even stronger contrast between Eulerian and moving frames due to the larger deformation and longer transient part of the signal. For this case, EG becomes markedly less compressible ($r_{99}=83$, $H_{\mathrm{norm}}=0.5250$), while LG remains highly compact ($r_{99}=15$, $H_{\mathrm{norm}}=0.1959$). The moving-Eulerian gas fraction (MEG) also remains far more compressible than EG, requiring only $r_{99}=19$ modes, although its entropy ($H_{\mathrm{norm}}=0.3080$) indicates a more distributed spectrum compared to LG in this strongly deforming case.
	
	Similar observations are noted in the case of the velocity field for C$_1$ and C$_3$. For C$_1$, the Eulerian velocity spectrum already exhibits moderate compressibility (EV: $r_{99}=10$, $H_{\mathrm{norm}}=0.2782$), and the Lagrangian transformation yields only a slight reduction in $r_{99}$ (LV: $r_{99}=8$) while the entropy remains comparable ($H_{\mathrm{norm}}=0.2845$). In contrast, the \hl{moving-Eulerian} velocity shows a clear improvement (MEV: $r_{99}=3$, $H_{\mathrm{norm}}=0.1328$), demonstrating a stronger concentration of velocity energy when mean advection is removed. For C$_3$, the same trend persists: EV requires $r_{99}=21$ modes ($H_{\mathrm{norm}}=0.2665$), LV decreases this to $r_{99}=14$ but with a comparable entropy level ($H_{\mathrm{norm}}=0.2971$), whereas MEV achieves improved compressibility ($r_{99}=10$, $H_{\mathrm{norm}}=0.2205$).
	
	Finally, the Lagrangian position variable is also highly compact, with LP requiring $r_{99}=7$ modes ($H_{\mathrm{norm}}=0.1744$) for C$_1$ and $r_{99}=10$ modes ($H_{\mathrm{norm}}=0.1957$) for C$_3$. Overall, based on the spectral compressibility metrics, one may conclude that the Lagrangian and \hl{moving-Eulerian} frameworks consistently outperform the Eulerian framework. Moreover, the advantage of working in a moving frame becomes more pronounced when the bubble undergoes stronger deformation for the gas volume fraction field; for the velocity field, the \hl{moving-Eulerian} framework provides the clearest and most consistent reduction in spectral complexity.
	...
	\begin{figure}[!ht]
		...
		\begin{subfigure}[t]{1\textwidth}
			\centering
			\includegraphics[width=\linewidth,
			trim={0cm 0cm 0cm 0cm},clip]{Figure4a.eps}
		\end{subfigure}
		\hspace{0.01\textwidth}
		\begin{subfigure}[t]{1\textwidth}
			\centering
			\includegraphics[width=\linewidth,
			trim={0cm 0cm 0cm 1cm},clip]{Figure4b.eps}
		\end{subfigure}
	...
	\begin{figure}[!ht]
		\centering
		\begin{subfigure}[t]{1.02\textwidth}
			\centering
			\includegraphics[width=\linewidth,
			trim={0.0cm 0cm 0cm 0cm},clip]{Figure5a.eps}
		\end{subfigure}
		\hspace{0.01\textwidth}
		\begin{subfigure}[t]{1.02\textwidth}
			\centering
			\includegraphics[width=\linewidth,
			trim={0cm 0cm 0cm 1cm},clip]{Figure5b.eps}
		\end{subfigure}
	...
	\section{Rise velocity calculation in Lagrangian frame}
	\label{Append_A}
