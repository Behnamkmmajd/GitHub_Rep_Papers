...
col_L   = [0.0  0.0  0.82];   % Lagrangian      = blue
...
snap_interface = [1, 15, 450];   % <-- replace with desired frames
...
P_SL = load(fullfile('grid_generator_case_parameters', 'Re168Bo12.mat'));
...
%% ================================================================
%  FIGURE 1a — XY planar trajectory  (with DNS bubble contours)
%  ================================================================
...
for k = snap_iso(:)'
    ...
    G_dns  = h5read(LEG_file_POD, [grp_EXACT, file_name]);
    G_grid = permute(reshape(G_dns, [nx_LE, ny_LE, nz_LE]), [2, 1, 3]);
    G_slice = G_grid(:,:, slice_z);

    X_slice = Xg(:,:, slice_z);
    Y_slice = Yg(:,:, slice_z);

    contourf(ax_xy, X_slice, Y_slice, G_slice, linspace(contour_min, 1, 20), 'LineStyle', 'none');
end

% Trajectory lines
h_pod_xy = plot(ax_xy, centroid_pod(:,1), centroid_pod(:,2), ...
    'Color', col_pod, 'LineStyle', ls_POD, 'LineWidth', lw);
h_dmd_xy = plot(ax_xy, centroid_dmd(:,1), centroid_dmd(:,2), ...
    'Color', col_dmd, 'LineStyle', ls_DMD, 'LineWidth', lw);
h_dns_xy = plot(ax_xy, centroid_dns(:,1), centroid_dns(:,2), ...
    'Color', col_dns, 'LineStyle', ls_DNS, 'LineWidth', lw_DNS);
uistack(h_dns_xy, 'top');

colormap(ax_xy, flipud(gray));
clim(ax_xy, [0 1]);
daspect(ax_xy, [1 1 1]);
xlim([2.5 7.5])
ylim([25 50])
%xlabel(ax_xy, '{\itX} / {\itD}');
%ylabel(ax_xy, '{\itY} / {\itD}');
set(ax_xy, ax_style{:});
%legend([h_pod_xy, h_dns_xy, h_dmd_xy], {'ME-POD', 'DNS', 'ME-DMD'}, 'FontSize', 22, 'FontName', 'Times New Roman', 'Location', 'best');
drawnow
...
for s = 1:nSnap
    ...
    clim(ax, [0 1]);
    daspect(ax, [1 1 1]);
    text(ax, 0.05, 0.07, sprintf('\\tau_%d = %.2f', s ,tau_k), ...
...
xlim(ax_sp, [0 50]);
ylim(ax_sp, [0.8 1.005]);
xlabel(ax_sp, '{\it\tau}');
%ylabel(ax_sp, 'Sphericity');
set(ax_sp, ax_style{:});

annotation(gcf,'rectangle',...
    [0.131339539978094 0.162291169451074 0.0636221248630888 0.742243436754179],...
    'LineWidth',3,...
    'LineStyle','--');
...
% 
% print(fig1a, fullfile(scriptDir, 'Figure6a2.eps'), '-depsc2', '-vector');
% print(fig1b, fullfile(scriptDir, 'Figure6b2.eps'), '-depsc2', '-vector');
% print(fig2,  fullfile(scriptDir, 'Figure6c2.eps'), '-depsc2', '-vector');
% print(fig3,  fullfile(scriptDir, 'Figure6d2.eps'), '-depsc2', '-vector');
% 
export_data = struct();
...
% fprintf('--- EPS export complete: Figure6a2-Figure6d2 ---\n');
