...
% Lagrangian (interpolated): volume-averaged + centroid, one curve per case
...
for i = startNum:endNum
    ...
    %======================================================================
    %  LAGRANGIAN FRAMES: interpolate scattered G, V onto Eulerian grid
    %======================================================================
    for k = 1:nLag
        ...
        %------------------------------------------------------------------
        v_L_field(i, k) = rise_velocity_mean_alpha(G_int, Xg_E, V_int);
            circ_L(i, k) = circularity_alpha_grid(G_int, x_E, y_E, circ_level);
        ...
        if i == startNum
            [yc_L(i, k), v_L_centroid(i, k)] = rise_velocity_centroid_alpha( ...
                G_int, Xg_E, Yg_E, NaN, time(i), time(i));
        else
            [yc_L(i, k), v_L_centroid(i, k)] = rise_velocity_centroid_alpha( ...
                G_int, Xg_E, Yg_E, yc_L(i-1, k), time(i), time(i-1));
        end
    end
...
set(ax1, 'FontName', 'Times New Roman', 'FontSize', 38, 'FontWeight', 'bold', 'LineWidth', 5.5);
...
set(ax2, 'FontName', 'Times New Roman', 'FontSize', 38, 'FontWeight', 'bold', ...
    'XMinorTick', 'on', 'YMinorTick', 'on', 'LineWidth', 5.5);
legend(ax2,'DNS', 'Lagrangian', ...
       'Orientation','vertical','NumColumns',1,...
       'FontSize',34,'FontName','Times New Roman');



save('cache_Fig2a_Vcubic_Gcubic.mat', 't_plot','idx','v_E_field','v_E_centroid','v_L_field', ...
