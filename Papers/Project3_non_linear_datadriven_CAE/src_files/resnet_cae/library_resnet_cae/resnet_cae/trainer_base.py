...
from resnet_cae.metrics import MetricType, compute_metrics_single_array
...
class MyTrainerBase():
    ...
    def write_run_info(self, first_batch: torch.tensor, z: torch.tensor):
        ...
        """
        # z may be either shape (B, L) or (B, C, H, W)
        self.latent_num_pixels = z.shape[1] if len(z.shape) == 2 else z.shape[1] * z.shape[2] * z.shape[3]
        physical_num_pixels = first_batch.shape[1] * first_batch.shape[2] * first_batch.shape[3]
    ...
    def _evaluate_metrics_inner(self, dataloader, max_n_batches: int, split: str):
        ...
        Assumes:
         - samples are 2D (H, W) or multi-channel (C, H, W)
         - metrics are computed per-channel and averaged
        """
        metric_results = []
        ...
        for pred, data in self.run_inference(dataloader, max_n_batches):
            ...
            # Handle multi-channel data: compute metrics per-channel and average
            if data.ndim == 3:  # (C, H, W) multi-channel
                n_channels = data.shape[0]
                channel_metrics = []
                for c in range(n_channels):
                    channel_metrics.append(
                        compute_metrics_single_array(data[c], pred[c], self.metric_types)
                    )
                # Average metrics across channels
                row = {
                    k: np.mean([cm[k] for cm in channel_metrics])
                    for k in channel_metrics[0].keys()
                }
            else:  # 2D single-channel
        ...
        if self.accelerator.is_main_process:
            ...
            # PSD plots disabled (requires power_spectrum module)
    ...
    def write_intensity_histograms(self, preds, data, split: str):
        ...
        We have lists of 2D or 3D (multi-channel) arrays here. Note that we expect this to be called by main process only.
        """
        assert len(preds) > 0
        assert len(data) > 0
        assert preds[0].shape == data[0].shape
        assert preds[0].ndim in (2, 3)  # Support 2D (H, W) or 3D (C, H, W)

        outdir = self.results_folder / 'intensity_histograms'
        outdir.mkdir(exist_ok=True)

        # Handle multi-channel: plot histogram for each channel
        if preds[0].ndim == 3:
            n_channels = preds[0].shape[0]
            channel_names = ['G', 'U', 'V'] if n_channels == 3 else [f'Ch{i}' for i in range(n_channels)]
            
            fig, axs = plt.subplots(1, n_channels, figsize=(3 * n_channels, 3), dpi=150)
            if n_channels == 1:
                axs = [axs]
            
            for c in range(n_channels):
                ax = axs[c]
                ax.hist(data[:, c, :, :].flatten(), bins=100, alpha=0.5, label='Data', color='blue')
                ax.hist(preds[:, c, :, :].flatten(), bins=100, alpha=0.5, label='Preds', color='red')
                ax.set_yscale('log')
                ax.set_xlabel(f'Intensity ({channel_names[c]})')
...
def write_slice_plot(outpath: Path, data: np.ndarray, pred: np.ndarray):
    """Write a plot comparing original and reconstructed 2D images
    """
    assert data.shape == pred.shape, f"Expected shape {data.shape}, got {pred.shape}"
    assert len(data.shape) == 2, "Expected 2D data"
    ...
    im = ax.imshow(data, cmap="gray", vmin=data_min, vmax=data_max)
    fig.colorbar(im, ax=ax)

    ax = axs[1]
    ax.set_title('Reconstructed')
    im = ax.imshow(pred, cmap="gray", vmin=pred_min, vmax=pred_max)
    fig.colorbar(im, ax=ax)
